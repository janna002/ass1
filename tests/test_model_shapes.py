...
class TestMLPImplementation:
    ...
    def test_forward_pass(self, sample_config):
        ...
        output = model(x)
        assert output.shape == (8, 2), f"Expected (8, 2), got {output.shape}. Did you flatten?"
    ...
    def test_backprop(self, sample_config):
        ...
        assert not torch.equal(initial_val, param), "Weights did not update. Is the graph broken?"
