...
class TestPCAMPipeline:
    ...
    def test_numerical_stability(self, mock_data_dir):
        ...
        img, _ = ds[0]
        assert img.max() <= 255, "Image values > 255 found. Did you forget to clip before uint8 cast?"
    ...
    def test_heuristic_filtering(self, mock_data_dir):
        ...
        assert len(ds.indices) == 98, f"Filtering failed. Expected 98 samples, got {len(ds.indices)}"

    def test_dataloader_output_logic(self, mock_data_dir):
        """Verifies shapes, types, and label squeezing."""
        config = {"data": {"data_path": str(mock_data_dir), "batch_size": 4, "num_workers": 0}}
    ...
    def test_weighted_sampling(self, mock_data_dir):
        ...
        config = {"data": {"data_path": str(mock_data_dir), "batch_size": 40, "num_workers": 0}}
        train_loader, _ = get_dataloaders(config)
        ...
        assert positives > 12, f"WeightedSampler might not be working. Only {positives}/40 are class 1."
